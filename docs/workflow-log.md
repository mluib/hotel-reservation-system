...
Format: `[developer input] -> [what resulted]`

Kept lightweight, added as things happen. Used as raw material for a polished writeup at the end of the project.

This is the living version of the log — reconstructed and dated from the raw ChatGPT/Copilot exports in [`docs/raw-ai-logs/`](raw-ai-logs/) plus the git history, and maintained going forward per the standing instruction in [`CLAUDE.md`](../CLAUDE.md). [`docs/raw-ai-logs/`](raw-ai-logs/) remains untouched as the frozen original transcripts this was built from.
...
## 2026-08-18 — Phase 7: documentation restructuring and naming (Claude Code)

- Decided to convert `docs/` filenames to kebab-case, leaving `raw-ai-logs/` and root special files alone -> renamed `docs/ROADMAP.md`/`RUNNING.md`/`WORKFLOW_LOG.md`; separately renamed the `raw-ai-logs/` subfolders and the five auto-named Claude Code plan files to `phase-N-topic.md` themselves, catching one mislabeled Phase 5/6 file.
- Asked for a full consistency review — links, roadmap done-markers, pre-Phase-0 context, a workflow-log rule check, `running.md` restructured into three chapters -> fixed stale paths and formatting inconsistencies, added the pre-Phase-0 note, marked Phases 1/2/4/5/6 done; separately confirmed the old `phase-4-devops.md` plan's backend command no longer worked (missing `Jwt:Key`).
- Corrected "Copilot did the bulk of the implementation" as inaccurate ("parts") -> reworded across the docs.
- Pointed out the backend has no README so the frontend shouldn't either -> folded `frontend/README.md` into `docs/frontend.md`.

## 2026-08-18 — Phase 7: running.md rewrite (Claude Code)

- Asked for docker-compose/standalone-Docker/native ordering with per-chapter prerequisites, then over several rounds for standalone Docker examples, bold captions, and a full four-chapter restructure with a Visual Studio/VS Code split -> rewrote the doc through multiple correction rounds: fixed a self-contradictory "standalone via docker-compose" framing, matched the standalone example's connection string and JWT key to real values, explained port and environment-file defaults, and added build/launch/port-configuration bullets throughout.
- Asked for a full final review, approved two of six findings -> fixed the SQL-auth reasoning gap and a punctuation inconsistency, left the rest as intentional.
- Pointed out `1433`'s real collision risk with a locally-installed SQL Server, and wanted docker-compose's other conflicts noted in the README -> remapped docker-compose's db port to `14330` (updating the standalone example and connection syntax to match), and added a one-line port-conflict caveat to the README.

## 2026-08-18 — Phase 7: full documentation set and README rewrite (Claude Code)

- Asked for the whole project turned into a full documentation set — README leading with what/how/why, a technology summary as tables, AI *assisted* not *created* stated explicitly, full backend/DevOps understanding, frontend named as the AI-agentic exception — supplying a Word document of their own techniques as a structural draft -> cross-checked the document against the code, then wrote three new `docs/` files (`architecture-overview.md`, `decisions.md`, `ai-assisted-development.md`) plus a rewritten README with three tech tables, correcting one inaccuracy along the way (the draft's "Fluent Validation" is actually EF Core's Fluent API) and surfacing a stronger DevOps proof point than planned (the Docker/CI setup was developer-directed and run, Claude Code guidance-only).
- Pushed back on several framing issues across multiple review rounds: an "explained in the developer's own words" claim that would have been dishonest; an overstated "written by hand" DevOps claim (corrected to what's actually verifiable — developer directed/ran/debugged, Claude Code generated the file content); decision-ownership language reading as AI-decided rather than developer-decided, even when the AI raised the question; and an unnecessary defensive clause and unexplained jargon in the README's intro -> corrected all of it across the README, `decisions.md`, and `ai-assisted-development.md`.
- Reviewed the new docs against the actual code and asked for a naming-consistency pass -> found and fixed several structural inconsistencies: `frontend.md` existed with no `backend.md` equivalent (folded into `architecture-overview.md`); `roadmap.md` still named FluentValidation despite DataAnnotations being the real Stage 2 choice (added a matching `decisions.md` entry); four Backend-table inaccuracies (missing repository abstraction, Moq mis-scoped, vague `IncludeXmlComments` wording, missing `ExceptionHandlingMiddleware` reference); the frontend's design credit missing the Claude Code wireframe (mockup wasn't the only step); and every remaining plain-text file/folder reference across all seven docs, converted to real links.
- Asked whether prompt examples (like the ChatGPT-era archive's) were worth adding -> added an "Examples" chapter with two verbatim prompts from the frozen ChatGPT archive and three reconstructed-from-the-log Claude Code examples, chosen for genuine technical weight; later restructured into explicit Prompt/Result/Reviewed bullets per example, matching `decisions.md`'s format.
- Confirmed Phase 7 done -> marked it in `roadmap.md`.

## 2026-08-20 — Phase 7: documentation consistency follow-up (Claude Code)

- Questioned several wording choices across the diagram and README (business rules vs. invariants, Identity/JWT/authentication read as one concept, "Use cases" mislabeled as DDD, missing aggregate examples, OCP/LSP unexplained, the CI link's display path, whether Secrets belonged under DevOps) -> reworded the dependency diagram to say what each layer actually does instead of listing overlapping words, fixed the DDD/SOLID rows with real examples, and explained the reasoning behind the rest in chat.
- Asked for a full final consistency review across all seven files, then a pass to shorten and prune this log of low-impact entries -> fixed three more inconsistencies (a stale roadmap backlog line, a `decisions.md` claim pointing at an entry that didn't exist, an incomplete doc-index description); pointed out `roadmap.md` is a checklist, not a rationale doc, so trimmed a newly-added explanatory annotation back to a plain list; then, finding the Phase 7 portion of this log still disproportionately long next to the implementation-phase sections above, condensed it further — consolidating many single-edit bullets into the handful above, matching their density.

## 2026-08-20 — Post-Phase-7 follow-up: backend-unavailable screen and demo seeding (Claude Code)

- Flagged a rough edge: with no backend reachable, the frontend rendered anyway with misleading errors -> added backend-reachability detection and a dedicated "can't reach the server" screen, verified live with the backend deliberately stopped.
- Asked for demo seeding to cover three room types with photos and a meaningfully named hotel instead of generic placeholders -> extracted a dedicated seeder, added placeholder photos, and fixed a self-introduced bug where seeding also polluted the integration tests' database.
- Reported the documentation PR merged on GitHub -> pulled `main`, resolved one conflict, re-verified the full suite; also brought the README and `running.md` in line with the new features.
- Pointed out the seeded hotel's theming didn't match its described location -> renamed to Bussen Lodge with a matching photo.

## 2026-08-21 — Home page copy, room names, and a login/customer-deletion bug fix (Claude Code)

- Asked for warmer home page welcome copy with specific emphasis -> replaced the copy, iterated styling through several rounds to a final look.
...
## 2026-08-26 — Backend hardening pass, from an external cross-check review (Claude Code)

- Shared an external session's cross-check review of the finished repo and asked for a written plan on its six flagged gaps — problem, alternatives, a recommendation, and an effort estimate each — before deciding what to build -> verified every finding against the actual code and produced that six-point menu, with no code touched until each point was explicitly decided.
- Decided the double-booking race (point 1): a serializable transaction, tested against the existing SQLite-backed suite rather than standing up a real SQL Server test environment, deadlock-retry deliberately left out -> added an `ITransactionRunner` abstraction (keeping the Application layer free of EF Core), wrapped the reservation overlap-check-and-insert in it, and added a concurrency integration test firing two simultaneous create-requests at the same room/dates; a test failure along the way was traced to a genuine SQLite-specific behavior (a shared connection rejects a second transaction outright) and handled explicitly rather than papered over.
- Decided the registration/login enumeration asymmetry (point 2): document as an accepted trade-off rather than changing behavior, after establishing the "hide it" alternative would remove the weak-password signal a legitimate user needs -> added a `decisions.md` entry recording that reasoning.
- Decided login lockout and security headers (points 3 and 4): `SignInManager`-based lockout enforcement, and a hand-rolled headers/Kestrel fix over a new dependency for either -> wired `SignInManager` into the login flow with explicit lockout options, added a lockout integration test, and added response-header middleware plus Kestrel `Server`-header suppression.
- Decided image-upload validation and JWT storage (points 5 and 6), noting this round of work was scoped to the backend -> added a magic-byte signature check to image uploads (with new unit test coverage for a spoofed content-type), and added a `decisions.md` entry recording the JWT-in-localStorage trade-off rather than changing the frontend's auth model.
- Asked for the image-upload validation's two separate method calls to be combined into one -> merged them into a single `ImageValidation.ValidateAsync`.
